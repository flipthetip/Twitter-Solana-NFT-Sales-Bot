...
The ```GET/collections/:symbol/activities``` API call as of writing updates data approximately every 10 minutes. The default wait time for this bot has been updated accordingly to check for updates every 100 seconds. As a result NFT sales will have delays up to 100 seconds after the sale. We do not want to call irresponsibly to the API.
